...
function login()
{
   ...
   // }
    ...
    if (empty($formErrors)) {
        ...
        $pass = md5($password);
        $table = $role . 's';
        $query = $conn->query("SELECT * FROM $table where username='$username' And password='$pass'")or die($conn->error);
           ...
           //// if ($result) {
               ...
               // if ($row) {
                   ...
                   // if ($hashed_password) {
                        ...
                        header('location:' . $role . '/security_check.php');
                    } else {
...
function getQuestions($exam_id)
{
    $rows = array();
    global $conn;
    $stmt = $conn->prepare("SELECT * FROM questions WHERE exam_id = ? ORDER BY RAND()");
    if ($stmt) {
        $stmt->bind_param('i', $exam_id);
        $stmt->execute();
        $result = $stmt->get_result();
