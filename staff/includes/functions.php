...
function login()
{
   ...
   // }
    ...
    if (empty($formErrors)) {
        ...
        $pass = md5($password);
        $table = $role . 's';
        $query = $conn->query("SELECT * FROM check_in where username='$username' And password='$pass'")or die($conn->error);
...
function loggedAdmin()
{
    if (isset($_SESSION['user_id'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM check_in WHERE id = ? LIMIT 1");
...
function recentStudents()
{
    global $conn;
    $rows = array();
    $stmt = $conn->prepare("SELECT students.*, classes.name AS class_name FROM students INNER JOIN classes ON students.class_id=classes.id ORDER BY students.id DESC");
    if ($stmt) {
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result) {
